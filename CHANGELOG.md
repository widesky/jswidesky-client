...
## [Unreleased]
### CHANGED
- Client no longer throws a Axios error if a response has been received. Instead, the error found in the response
  as received from a WideSky API server is used as the error message. This has been changed as the WideSky API server
  already created good responses to request errors and changes should only be in API server.
